...
export function MarketingOverview({ model, loading, iq }) {
    ...
    return (<div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        {dashboardCards.map((card) => (<div key={card.label} className="rounded-2xl border border-white/[0.07] bg-[#0b0f17] p-4 transition-all duration-200 hover:border-gold/30">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        {operationalStats.map((stat) => (<Link key={stat.label} to={stat.to} className="group">
