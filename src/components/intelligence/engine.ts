/**
 * Motor de Inteligência Comercial — 100% cálculo local sobre dados já existentes.
 * Sem IA, sem novas regras de negócio, sem escrita no banco.
 */
import type { IntelligenceData } from "./useIntelligenceData";
...
export const daysBetween = (a: Date | string, b: Date | string = new Date()) =>
  Math.floor((+new Date(b) - +new Date(a)) / DAY);

const apptValue = (a: any) => n(a?.final_amount ?? a?.total_price);
const isCompleted = (a: any) => a?.status === "completed";
const isCancelled = (a: any) => a?.status === "cancelled" || a?.status === "no_show";
const isActiveAppt = (a: any) => !isCancelled(a);
...
export interface Intelligence {
  ...
  inactiveBuckets: { label: string; min: number; max: number | null; rows: CustomerRow[] }[];
  birthdays: { today: CustomerRow[]; week: CustomerRow[]; month: CustomerRow[] };
  vips: CustomerRow[];
  atRisk: { row: CustomerRow; reason: string }[];
  nearReward: { row: CustomerRow; reason: string }[];
  idle: { today: IdleSlot[]; tomorrow: IdleSlot[]; week: IdleSlot[] };
  products: {
    noSales: any[];
    topSellers: { name: string; count: number; total: number }[];
    lowStock: any[];
  };
  services: {
    rising: { name: string; current: number; previous: number; pct: number }[];
    falling: { name: string; current: number; previous: number; pct: number }[];
    lowVolume: { name: string; count: number }[];
    topRevenue: { name: string; total: number; count: number }[];
  };
  coupons: { active: any[]; expiring: any[]; mostUsed: any[]; neverUsed: any[] };
  cashback: { withBalance: CustomerRow[]; neverUsed: CustomerRow[] };
  credits: { withBalance: CustomerRow[]; expiringSoon: any[] };
  subscriptions: { renewing: any[]; lowUsage: any[]; unused: any[] };
  reviews: { negative: any[]; pendingReply: any[]; notReviewed: number };
  finance: {
    revenueThisWeek: number;
    revenueLastWeek: number;
    weekTrendPct: number | null;
    revenueThisMonth: number;
    ticketThisMonth: number;
    ticketLastMonth: number;
    ticketTrendPct: number | null;
    cancelRate30: number;
    occupancy7: number;
  };
  insights: { id: string; tone: "gold" | "positive" | "warning" | "neutral"; text: string }[];
  radar: RadarItem[];
}
...
export interface RadarItem {
  ...
  score: number;
  potential: number;
  priority: "muito-alta" | "alta" | "media" | "baixa";
  effort: "baixa" | "media" | "alta";
  action: { label: string; to?: string; search?: Record<string, any> };
}

const priorityOf = (score: number): RadarItem["priority"] =>
  score >= 80 ? "muito-alta" : score >= 60 ? "alta" : score >= 40 ? "media" : "baixa";

const sameDay = (a: Date, b: Date) =>
  a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();

export function buildIntelligence(data: IntelligenceData): Intelligence {
  const now = new Date();
  const {
    customers,
    appointments,
  ...
  const barberName = (id: any) => barbers.find((b) => b.id === id)?.name || null;
  const serviceName = (id: any) => services.find((s) => s.id === id)?.name || null;
  ...
  customers.forEach((c) => {
    ...
    const countBy = (key: string, resolver: (id: any) => string | null) => {
  ...
  // ——— Aniversariantes ———
  const inBirthdayRange = (r: CustomerRow, days: number) => {
    ...
    for (let i = 0; i <= days; i++) {
      ...
      if (ref.getMonth() === d.getUTCMonth() && ref.getDate() === d.getUTCDate()) return true;
    }
    return false;
  };
  const birthdays = {
  ...
  // ——— Em risco ———
  const atRisk: Intelligence["atRisk"] = [];
  allRows.forEach((r) => {
    if (r.visits < 2 || r.daysSince === null) return;
    const rows = (byCustomer[r.id] || []).sort((a, b) => +new Date(a.start_time) - +new Date(b.start_time));
    const gaps: number[] = [];
    for (let i = 1; i < rows.length; i++) gaps.push(daysBetween(rows[i - 1].start_time, rows[i].start_time));
  ...
  const cancelledRecent = appointments.filter(
    (a) => isCancelled(a) && daysBetween(a.cancelled_at || a.start_time) <= 30,
  );
  ...
  const nearReward: Intelligence["nearReward"] = [];
  allRows.forEach((r) => {
    if (r.cashback > 0) nearReward.push({ row: r, reason: `${brl(r.cashback)} de cashback disponível.` });
    else if (r.credits > 0) nearReward.push({ row: r, reason: `${brl(r.credits)} em créditos para usar.` });
    else if (r.visits > 0 && r.visits % 5 === 4) nearReward.push({ row: r, reason: "A 1 atendimento de completar 5 visitas." });
  });

  // ——— Horários ociosos ———
  const activeBarbers = barbers.filter((b) => b.active !== false);
  const SLOT = 30;
  const buildIdle = (date: Date): IdleSlot[] => {
    const dayAppts = appointments.filter(
      (a) => a.start_time && isActiveAppt(a) && sameDay(new Date(a.start_time), date),
    );
    const avgTicket =
      allRows.filter((r) => r.avgTicket > 0).reduce((s, r) => s + r.avgTicket, 0) /
  ...
  // ——— Produtos ———
  const soldMap: Record<string, { name: string; count: number; total: number }> = {};
  productSales
  ...
  const inWindow = (a: any, from: number, to: number) => {
    const d = daysBetween(a.start_time);
    return d >= to && d < from;
  };
  const svcAgg: Record<string, { name: string; current: number; previous: number; total: number; count: number }> = {};
  completed.forEach((a) => {
    ...
    if (inWindow(a, 30, 0)) svcAgg[name].current += 1;
    else if (inWindow(a, 60, 30)) svcAgg[name].previous += 1;
  });
  ...
  const couponsBlock = {
    ...
    expiring: coupons.filter(
      (c) => c.active && c.expires_at && daysBetween(now, c.expires_at) >= 0 && daysBetween(now, c.expires_at) <= 15,
    ),
  ...
  const creditsBlock = {
    ...
    expiringSoon: credits.filter(
      (c) =>
        n(c.available_amount) > 0 &&
        c.expires_at &&
  ...
  const subsBlock = {
    renewing: activeSubs.filter(
      (s) => s.current_period_end && daysBetween(now, s.current_period_end) >= 0 && daysBetween(now, s.current_period_end) <= 10,
    ),
  ...
  const sumWindow = (from: number, to: number) =>
    completed.filter((a) => inWindow(a, from, to)).reduce((s, a) => s + apptValue(a), 0);
  ...
  if (tomorrowFree > 0) insights.push({ id: "idle", tone: "neutral", text: `Você possui ${tomorrowFree} horários vagos amanhã.` });
  ...
  const push = (item: Omit<RadarItem, "priority">) => radar.push({ ...item, priority: priorityOf(item.score) });
