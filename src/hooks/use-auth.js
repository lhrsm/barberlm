...
function emit() {
    listeners.forEach((l) => l({ user: globalUser, session: globalSession, profile: globalProfile, loading: globalLoading }));
}
function setState(partial) {
    if ('user' in partial)
        globalUser = partial.user;
    if ('session' in partial)
        globalSession = partial.session;
    if ('profile' in partial)
        globalProfile = partial.profile;
    if ('loading' in partial)
        globalLoading = partial.loading;
    emit();
}
async function fetchProfileData(userId) {
    try {
        ...
        const resolvedRole = roleData?.role ?? profileData?.role ?? null;
