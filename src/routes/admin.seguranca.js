...
import { ShieldCheck, Lock, Key, History, FileLock, AlertTriangle, Fingerprint, Shield, CheckCircle2, ArrowRight, Database, Globe } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="bg-[#0b0f17] border-zinc-800/80 shadow-2xl flex flex-col">
          ...
          <CardContent className="flex-1 space-y-3">
            {overview?.alerts?.map((alert) => (<div key={alert.id} className="p-3 bg-zinc-900/50 border border-zinc-800 rounded-xl flex items-start gap-3 hover:border-amber-500/30 transition-all cursor-pointer group">
            ...
            {(!overview?.alerts || overview.alerts.length === 0) && (<div className="h-full flex flex-col items-center justify-center py-8 text-center">
        ...
        <PremiumTabsBody>
          <PremiumTabsContent value="overview">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <SecurityModuleCard title="Proteção de Identidade" description="Gestão de sessões, MFA e controle de dispositivos ativos." icon={Fingerprint} status="Ativo" items={["Políticas de Senha", "Sessões Ativas", "MFA Adaptativo (Beta)"]}/>
              <SecurityModuleCard title="Isolamento Multi-tenant" description="RLS auditado e guards de proteção cross-tenant." icon={Lock} status="Blindado" items={["Tabelas RLS", "RPC Guards", "Token Mapping"]}/>
              <SecurityModuleCard title="Conformidade LGPD" description="Governança de dados, consentimentos e anonimização." icon={FileLock} status="Pendente" items={["Inventário de Dados", "Portabilidade", "Direito ao Esquecimento"]}/>
              <SecurityModuleCard title="Auditoria Proativa" description="Logs imutáveis de todas as operações administrativas." icon={History} status="Logando" items={["Audit Logs", "Alertas de Risco", "Impersonation Tracker"]}/>
            </div>
          </PremiumTabsContent>
...
function SecurityIndicator({ icon: Icon, label, status }) {
    return (<div className="flex flex-col items-center gap-2 p-2 rounded-xl hover:bg-zinc-900/50 transition-colors cursor-default">
      <div className={cn("h-10 w-10 rounded-xl border grid place-items-center", status === 'success' ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-500" : "bg-amber-500/5 border-amber-500/20 text-amber-500")}>
        <Icon size={20}/>
      </div>
...
function SecurityModuleCard({ title, description, icon: Icon, status, items }) {
    return (<Card className="bg-[#0b0f17] border-zinc-800/80 hover:border-indigo-500/30 transition-all group">
      <CardHeader className="pb-2">
      ...
      <CardContent>
        <ul className="space-y-2">
          {items.map((item, i) => (<li key={i} className="flex items-center gap-2 text-[10px] font-bold text-zinc-400 uppercase italic">
