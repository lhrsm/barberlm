...
import { Zap, CreditCard, MessageSquare, BarChart3, Webhook, Settings2, ShieldCheck, AlertTriangle, CheckCircle2, XCircle, Clock, Calendar, Layers, Activity } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { PremiumTabs, PremiumTabsList, PremiumTabsBody, PremiumTabsContent } from "@/components/ui/premium-tabs";
...
function IntegrationsCenterPage() {
    ...
    return (<div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {categories.map(cat => (<Card key={cat.id} className="bg-[#0b0f17] border-zinc-800/80 hover:border-emerald-500/30 transition-all group">
        ...
        <PremiumTabsBody>
          <PremiumTabsContent value="catalog">
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <IntegrationCard name="Stripe" provider="stripe" description="Gateway oficial para assinaturas e infraestrutura SaaS." status={health?.payments?.find(p => p.provider === 'stripe')?.status || 'active'} environment={health?.payments?.find(p => p.provider === 'stripe')?.environment || 'live'} category="Pagamentos" icon={<CreditCard className="text-sky-400"/>}/>

                {/* Z-API Integration Card */}
                <IntegrationCard name="Z-API (WhatsApp)" provider="zapi" description="Motor de automações e mensagens transacionais via WhatsApp." status={health?.whatsapp?.status || 'active'} health={health?.whatsapp?.health} category="Comunicação" icon={<MessageSquare className="text-emerald-400"/>}/>

                {/* Mercado Pago Integration Card */}
                <IntegrationCard name="Mercado Pago" provider="mercadopago" description="Recebimento de PIX e cartões para clientes finais." status={health?.payments?.find(p => p.provider === 'mercadopago')?.status || 'not_configured'} category="Pagamentos" icon={<CreditCard className="text-blue-400"/>}/>

                {/* Futuros */}
                <IntegrationCard name="Google Calendar" provider="google" description="Sincronização de agenda profissional com Google." status="coming_soon" category="Agenda" icon={<Calendar className="text-rose-400"/>}/>
             </div>
          </PremiumTabsContent>
             ...
             <Card className="bg-[#0b0f17] border-zinc-800/80">
                ...
                <CardContent>
                   <div className="space-y-4">
                      {isLoading ? (<div className="p-8 text-center text-zinc-500 italic">Carregando telemetria...</div>) : (health?.payments?.map(p => (<div key={p.id} className="flex items-center justify-between p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                            <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-6">
                              ...
                              <Badge className={cn("h-6 px-3 font-black uppercase text-[10px] tracking-widest", p.status === 'active' ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border-rose-500/20")}>
...
function IntegrationCard({ name, description, status, health, category, icon, environment }) {
    const statusConfig = {
    ...
    return (<Card className="bg-[#0b0f17] border-zinc-800/80 hover:border-emerald-500/30 transition-all flex flex-col h-full shadow-xl">
      <CardHeader className="pb-4">
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            {environment && (<Badge variant="secondary" className="h-5 px-2 text-[8px] font-black uppercase bg-zinc-800 text-zinc-400 border-none tracking-tighter">
                {environment}
              </Badge>)}
          </div>
        </div>
      ...
      <CardContent className="flex-1 pb-6">
        ...
        {health === 'degraded' && (<div className="mt-4 p-2 bg-amber-500/5 border border-amber-500/20 rounded-lg flex items-center gap-2">
