...
function ResetPasswordPage() {
    ...
    return (<div className="min-h-screen w-full flex items-center justify-center bg-[#050b18] p-4 relative overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="w-full max-w-md relative z-10 flex flex-col items-center">
        ...
        <div className="bg-white rounded-[32px] shadow-2xl overflow-hidden p-8">
          <AnimatePresence mode="wait">
            {status === 'loading' && (<div className="flex flex-col items-center justify-center py-12 space-y-4">
                <Loader2 className="w-10 h-10 animate-spin text-gold"/>
                <p className="text-zinc-500 text-xs font-black uppercase tracking-widest">Validando acesso...</p>
              </div>)}

            {status === 'form' && (<motion.div key="form" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" {...form.register("password")} autoComplete="new-password" className="h-14 pl-12 pr-12 bg-white border-zinc-200 rounded-2xl text-black focus-visible:ring-gold/10 focus-visible:border-gold/60 transition-all"/>
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-black transition-colors">
                        {showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}
                      </button>
                    </div>
                    {form.formState.errors.password && (<p className="text-[10px] text-red-500 font-bold ml-1">{form.formState.errors.password.message}</p>)}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-gold transition-colors" size={18}/>
                      <Input id="confirmPassword" type={showPassword ? "text" : "password"} placeholder="••••••••" {...form.register("confirmPassword")} autoComplete="new-password" className="h-14 pl-12 bg-white border-zinc-200 rounded-2xl text-black focus-visible:ring-gold/10 focus-visible:border-gold/60 transition-all"/>
                    </div>
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-14 rounded-2xl bg-black text-white font-black uppercase tracking-widest hover:bg-zinc-800 transition-all shadow-lg shadow-black/10 active:scale-[0.98]">
                    {loading ? <Loader2 className="animate-spin"/> : "Atualizar senha"}
                  </Button>
                </form>
              </motion.div>)}

            {status === 'success' && (<motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6 py-4">
                ...
                <Button asChild className="w-full h-14 rounded-2xl bg-black text-white font-black uppercase tracking-widest hover:bg-zinc-800 transition-all">
                  <Link to="/auth" search={{ tab: "login" }}>Entrar no Barbex</Link>
                </Button>
              </motion.div>)}

            {status === 'expired' && (<motion.div key="expired" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6 py-4">
                ...
                <Button asChild variant="outline" className="w-full h-14 rounded-2xl border-zinc-200 font-black uppercase tracking-widest">
