...
function AutomationsComponent() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const { count: totalCallbacks } = await anySupabase.from("automation_v2_dispatches").select("*", { count: 'exact', head: true }).eq("tenant_id", tenantId).eq("requires_callback", true).eq("callback_received", true);
      ...
      const { count: pendingCallbacksCount } = await anySupabase.from("automation_v2_dispatches").select("*", { count: 'exact', head: true }).eq("tenant_id", tenantId).eq("requires_callback", true).eq("callback_received", false);
      ...
      setLogStats({
        ...
        lastUpdate: new Date(),
        pendingCallbacks: pendingCallbacksCount || 0,
        callbacksReceived: totalCallbacks || 0,
        dispatchesCreated: totalDispatches || 0,
        sessionsCreated: totalSessions || 0
      });
                    ...
                    <div className="mx-6 mt-4 p-4 rounded-2xl bg-slate-900/50 border border-white/5 shadow-inner">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-10 gap-6">
                        <div className="space-y-1.5">
                          <p className="text-[9px] text-slate-500 uppercase font-bold">Dispatch Created</p>
                          <p className="text-[10px] font-mono text-emerald-400">TRUE</p>
                        </div>
                        <div className="space-y-1.5">
                          <p className="text-[9px] text-slate-500 uppercase font-bold">Session Created</p>
                          <p className="text-[10px] font-mono text-emerald-400">{logs[0].session_id ? 'TRUE' : 'FALSE'}</p>
                        </div>
                        <div className="space-y-1.5">
                          <p className="text-[9px] text-slate-500 uppercase font-bold">Provider MSG ID</p>
                          <p className="text-[10px] font-mono text-white truncate">{logs[0].provider_message_id || logs[0].message_id || 'N/A'}</p>
                        </div>
                        <div className="space-y-1.5">
                        ...
                        <div className="space-y-1.5">
                          <p className="text-[9px] text-slate-500 uppercase font-bold">Workflow</p>
                          <p className="text-[10px] font-mono text-white">{logs[0].workflow_key || 'N/A'}</p>
                        </div>
                        <div className="space-y-1.5">
                        ...
                        <tr key={log.id} className={`${idx % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.02]'} hover:bg-white/[0.05] transition-colors group`}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              <span className="text-sm font-medium text-white">{new Date(log.sent_at || log.created_at).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</span>
                              <span className="text-[10px] text-slate-500 font-mono">{new Date(log.sent_at || log.created_at).toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</span>
                            </div>
                          </td>
            ...
            <div className="p-8 pt-2 space-y-8 overflow-y-auto max-h-[calc(90vh-120px)] custom-scrollbar">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {(selectedLog.status === 'failed' || selectedLog.status === 'error') && (
                    <Button 
                      size="sm" 
                      className="h-8 bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold gap-2 rounded-lg transition-all shadow-lg shadow-amber-500/20"
                      onClick={() => handleResendWhatsApp(selectedLog)}
                ...
                <div className="bg-[#081229] border border-white/5 rounded-2xl p-4 overflow-hidden shadow-xl">
                   {loadingDeliveryLogs ? (
                     ...
                     <div className="p-8 text-center text-slate-500 italic text-sm">
                       Nenhum registro de auditoria detalhado encontrado para este envio.
                     </div>
                   ) : (
                     <div className="space-y-3">
                       {deliveryLogs.map((audit, i) => (
                         <div key={audit.id} className="flex gap-4 p-3 rounded-xl bg-white/[0.02] border border-white/5 items-start">
                           <div className="flex flex-col items-center gap-1 mt-1">
                             <div className={cn(
                               "w-2.5 h-2.5 rounded-full",
                               audit.status === 'success' || audit.status === 'sent' || audit.status === 'delivered' ? 'bg-emerald-500' :
                               audit.status === 'failed' ? 'bg-rose-500' : 'bg-amber-500'
                             )} />
                             {i < deliveryLogs.length - 1 && <div className="w-0.5 h-8 bg-white/5" />}
                           </div>
                           <div className="flex-1 space-y-1">
                             <div className="flex justify-between">
                               <div className="flex items-center gap-2">
                                 <span className="text-xs font-bold text-white uppercase tracking-wider">
                                   {audit.status === 'success' ? 'Sucesso' : audit.status === 'failed' ? 'Falha' : audit.status}
                                 </span>
                                 {audit.attempt_number && (
                                   <Badge variant="outline" className="text-[9px] h-4 bg-white/5 border-white/10 text-slate-400">
                                     Tentativa #{audit.attempt_number}
                                   </Badge>
                                 )}
                               </div>
