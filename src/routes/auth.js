...
import { CheckCircle2, ShieldCheck, ChevronLeft, Sparkles, Calendar, MessageSquare, Award, Zap } from "lucide-react";
...
function AuthPageComponent() {
    ...
    return (<div className="min-h-screen w-full flex flex-col md:flex-row bg-[#05070d] overflow-hidden relative selection:bg-gold selection:text-black font-sora">
      {/* Voltar Button */}
      <div className="absolute top-6 left-6 z-50">
        <Button variant="ghost" size="sm" onClick={() => navigate({ to: "/" })} className="text-white/60 hover:text-gold transition-colors gap-2 font-bold uppercase tracking-widest text-[10px]">
          <ChevronLeft size={16}/>
          Voltar
        </Button>
      </div>
      ...
      <div className="absolute inset-0 z-0 hidden md:block w-[60%]">
        <img src="https://images.unsplash.com/photo-1503951914875-452162b0f3f1?q=80&w=2070&auto=format&fit=crop" alt="Barbershop" className="w-full h-full object-cover opacity-30 mix-blend-luminosity grayscale"/>
      ...
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="relative z-10 w-full md:w-[60%] flex flex-col justify-center px-8 md:px-24 py-20 md:py-0">
        ...
        <div className="max-w-xl space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8 pt-4">
            {benefits.map((benefit, idx) => (<div key={idx} className="flex items-center gap-3 group">
      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="relative z-20 w-full md:w-[40%] min-w-[320px] max-w-full md:max-w-[550px] flex items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-[430px] md:max-w-[480px]">
          ...
          <div className="relative group">
            ...
            <div className={`relative ${isClientPortal ? 'bg-white' : 'bg-[#0d0f14]/80 backdrop-blur-xl border border-white/5'} rounded-[32px] shadow-2xl overflow-hidden p-8 md:p-10`}>
              ...
              {isClientPortal ? (<div className="space-y-6">
                  {!migrationData ? (<ClientLoginForm onMigrationRequired={(data) => setMigrationData(data)}/>) : (<BookingAuthStep customerName="Cliente Barbex" customerPhone={migrationData.phone || ""} customerId={null} tenantId="" onSuccess={() => window.location.reload()} onBack={() => setMigrationData(null)}/>)}
                  <div className="text-center pt-4">
                    <Button variant="link" onClick={() => navigate({ to: '/auth', search: { tab: undefined } })} className="text-zinc-400 hover:text-gold text-[10px] font-black uppercase tracking-widest">
                  ...
                  <div className="text-center pt-6">
                    <Button variant="link" onClick={() => navigate({ to: '/auth', search: { tab: 'client' } })} className="text-zinc-500 hover:text-gold text-[10px] font-black uppercase tracking-widest">
        ...
        ].map((info, idx) => (<div key={idx} className="flex items-center gap-2">
