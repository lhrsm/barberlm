                    ...
                    <SelectContent className="bg-[#0b0f17] border-[#1f2937] text-white">
                      ...
                      {customers.map((c) => (
                        <SelectItem key={c.id} value={c.id} className="focus:bg-amber-500/20">{c.name}</SelectItem>
                                ...
                                <div className="space-y-2">
                                  <Label htmlFor="new-customer-id" className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Cliente (Opcional)</Label>
                                  <Select 
                                    value={newTransaction.customer_id} 
                                    onValueChange={(val) => setNewTransaction({...newTransaction, customer_id: val})}
                                  >
                      ...
                      <TableRow key={req.id} className="border-border hover:bg-muted/50 transition-colors">
                        ...
                        <TableCell className="font-bold">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-zinc-500 font-normal uppercase">{req.appointment?.service_name || "Serviço"}</span>
                          </div>
                        </TableCell>
                        <TableCell className="text-xs">
                          {req.pix_key ? (
                            <div className="flex flex-col">
                              ...
                              <span className="text-[9px] text-zinc-500 uppercase">{req.pix_type} - {req.holder_name}</span>
                            </div>
                          ) : (
                              ...
                              <DialogContent className="bg-[#0b0f17] border-zinc-800 text-white max-w-md">
                                ...
                                <div className="space-y-4 my-4">
                                  <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-2">Dados do Pagamento</p>
                                    <p className="text-sm"><strong>Método:</strong> {req.payment_method || 'Pix'}</p>
                                    <p className="text-sm"><strong>ID Pagamento:</strong> <span className="text-xs text-zinc-400 break-all">{req.payment_id || 'N/A'}</span></p>
                                    {req.pix_key && (
                                      <>
                                        <div className="h-px bg-zinc-800 my-2" />
                                        <p className="text-sm"><strong>Chave Pix:</strong> {req.pix_key}</p>
                                        <p className="text-sm"><strong>Tipo:</strong> {req.pix_type}</p>
                                        <p className="text-sm"><strong>Titular:</strong> {req.holder_name}</p>
                                      </>
                                    )}
                                  </div>
                                  <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Histórico de Alterações</p>
