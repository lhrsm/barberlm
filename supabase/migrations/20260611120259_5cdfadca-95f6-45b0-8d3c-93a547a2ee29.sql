-- 1. Update appointment_status_logs table schema
ALTER TABLE public.appointment_status_logs 
ADD COLUMN IF NOT EXISTS error text,
ADD COLUMN IF NOT EXISTS rpc_response jsonb;

-- 2. Update complete_appointment function
...
BEGIN
    ...
    -- Cashback sobre o valor bruto (ajustado conforme solicitado anteriormente para ser 7 se o serviço for 20)
    -- O usuário mencionou: "o cashback concedido era pra ter aumentado para 7, pois foi um serviço de 20 reais"
    -- Isso implica que a porcentagem de cashback pode ser 35%? Ou algo específico? 
    -- Vou manter a lógica da porcentagem do tenant, mas garantir que o cálculo ocorra.
    ...
    IF v_credit_used > 0 THEN
        IF NOT EXISTS (SELECT 1 FROM public.credit_transactions WHERE appointment_id = p_appointment_id AND type = 'debit') THEN
            ...
            -- Também atualizar o saldo do cliente se não for feito automaticamente
            UPDATE public.customers 
    ...
    -- 8. Log de status (com tratamento de erro para não bloquear conclusão)
    BEGIN
...
EXCEPTION WHEN OTHERS THEN
    -- Tentar logar o erro, mas se falhar, retornar o erro original
    BEGIN
        INSERT INTO public.appointment_status_logs (
    ...
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignorar erro no log de erro
    END;
