-- 1. Create enum for time off types
DO $$ 
BEGIN
...
-- 2. Create professional_time_off table
CREATE TABLE IF NOT EXISTS public.professional_time_off (
    ...
    type public.time_off_type NOT NULL DEFAULT 'personal_block',
    title TEXT,
    description TEXT,
...
-- 5. Policies
CREATE POLICY "Users can view time off for their tenant"
ON public.professional_time_off FOR SELECT
TO authenticated
USING (
    tenant_id = auth.uid() 
    OR 
    professional_id IN (SELECT id FROM public.barbers WHERE user_id = auth.uid())
);

CREATE POLICY "Admins can manage time off for their tenant"
ON public.professional_time_off FOR ALL
TO authenticated
USING (tenant_id = auth.uid())
WITH CHECK (tenant_id = auth.uid());

-- 6. Updated get_availability_slots to include time_off
...
-- 7. RPC to check conflicts with existing appointments
CREATE OR REPLACE FUNCTION public.check_time_off_conflicts(
