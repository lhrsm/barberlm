...
BEGIN
    ...
    -- 5. Cashback (Geração)
    -- Melhorado para verificar qualquer transação de crédito (ganho) para este agendamento
    SELECT EXISTS (
        SELECT 1 FROM public.cashback_transactions 
        WHERE appointment_id = p_appointment_id 
        AND type IN ('credit', 'earned', 'cashback_earned', 'granted')
    ...
    IF v_tenant.cashback_enabled AND v_cashback_base_amount >= COALESCE(v_tenant.cashback_minimum_amount, 0) AND NOT v_existing_cashback THEN
        ...
        IF v_cashback_earned > 0 THEN
            -- ON CONFLICT para segurança extra contra duplicidade
            ...
            -- Só atualizar o saldo se a inserção acima foi bem sucedida? 
            -- O ON CONFLICT DO NOTHING não nos diz se inseriu ou não sem checar rows.
            -- Mas como já checamos v_existing_cashback, estamos mais seguros.
            -- Para garantir que não duplique saldo se houver race condition:
            IF NOT EXISTS (
                SELECT 1 FROM public.cashback_transactions 
                WHERE appointment_id = p_appointment_id 
                AND type = 'credit'
                AND created_at > now() - interval '10 seconds' -- Verificação de segurança rápida
            ) THEN
                -- Como o ON CONFLICT pode ter impedido a inserção agora, 
                -- e o saldo deve ser atualizado apenas se inseriu, 
                -- o ideal é que a lógica de saldo esteja em um trigger na cashback_transactions.
                -- Se não estiver, esta parte ainda pode ser perigosa em race conditions severas, 
                -- mas o check v_existing_cashback já resolve 99% dos casos.
    ...
    IF v_credit_used > 0 THEN
        IF NOT EXISTS (SELECT 1 FROM public.credit_transactions WHERE appointment_id = p_appointment_id AND type = 'debit') THEN
            ...
            UPDATE public.customers 
            SET loyalty_credits = GREATEST(0, COALESCE(loyalty_credits, 0) - v_credit_used),
                updated_at = now()
            WHERE id = v_appt.customer_id;
        END IF;
    END IF;
    ...
    SET 
        ...
        cashback_earned = COALESCE(v_cashback_earned, cashback_earned),
        completed_at = now(),
        updated_at = now()
    ...
    BEGIN
        INSERT INTO public.appointment_status_logs (
            appointment_id, old_status, new_status, changed_by_type, changed_by_id, source, rpc_response, metadata
        ) VALUES (
            p_appointment_id, v_status_before, 'completed', p_changed_by_type, p_changed_by_id, p_source, 
            jsonb_build_object('success', true, 'cashback_earned', v_cashback_earned, 'final_amount', v_final_amount),
            p_metadata
        );
        v_log_created := true;
...
EXCEPTION WHEN OTHERS THEN
    BEGIN
        INSERT INTO public.appointment_status_logs (
            appointment_id, old_status, new_status, changed_by_type, changed_by_id, source, error, metadata
        ) VALUES (
            p_appointment_id, COALESCE(v_status_before, 'unknown'), 'error', p_changed_by_type, p_changed_by_id, p_source, SQLERRM, p_metadata
        );
    EXCEPTION WHEN OTHERS THEN
        NULL;
    END;
    ...
    RETURN jsonb_build_object(
        ...
        'error', SQLERRM,
        'appointment_id', p_appointment_id,
        'status_before', COALESCE(v_status_before, 'unknown'),
        'status_after', 'error'
    );
END;
$function$