...
BEGIN
  ...
  IF v_template_id IS NOT NULL AND v_active = TRUE THEN
    ...
    v_appointment_date := to_char(NEW.start_time, 'DD/MM/YYYY');
    v_appointment_time := to_char(NEW.start_time, 'HH24:MI');
    ...
    INSERT INTO public.automation_queue (
      ...
      jsonb_build_object(
        ...
        'rendered', v_template -- Original template as fallback or for reference
      ),
      v_idempotency_key
