...
DECLARE
    ...
    v_already_processed_pix BOOLEAN;
    v_result JSONB;
BEGIN
    ...
    ELSE
        -- Fallback: Total - (Credits + Cashback)
    ...
    SET
        ...
        refund_status = CASE 
            ...
            WHEN p_refund_preference = 'refund' THEN 'requested'
            ELSE 'pending'
        END,
        refund_type = CASE 
    ...
    -- 5. Automatic reversal of Credits (that were used for payment)
    IF v_credits_used > 0 THEN
        ...
        IF NOT v_already_reversed_credits THEN
            ...
            INSERT INTO public.credit_transactions (
                ...
                'credit_reversed',
                v_credits_used,
                'R$ ' || v_credits_used || ' em créditos foram devolvidos devido ao cancelamento do agendamento.'
            );
            
            -- Financial adjustment log (credit_reversed)
    ...
    -- 6. Automatic reversal of Cashback (that was used for payment)
    IF v_cashback_used > 0 THEN
        ...
        IF NOT v_already_reversed_cashback THEN
            ...
            INSERT INTO public.cashback_transactions (
                ...
                'cashback_reversed',
                v_cashback_used,
                'R$ ' || v_cashback_used || ' de cashback foi devolvido devido ao cancelamento do agendamento.'
            );
            
            -- Financial adjustment log (cashback_reversed)
            INSERT INTO public.transactions (
                user_id,
                tenant_id,
                appointment_id,
    ...
    IF v_is_pix_paid AND v_pix_amount > 0 THEN
        -- Check if already processed Pix refund/credit for this appointment
        SELECT EXISTS (
            SELECT 1 FROM public.transactions 
            WHERE appointment_id = p_appointment_id 
            AND type IN ('credit_granted', 'refund_requested')
        ) INTO v_already_processed_pix;

        IF NOT v_already_processed_pix THEN
