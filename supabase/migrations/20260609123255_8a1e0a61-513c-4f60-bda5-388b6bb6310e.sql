...
AS $function$
 ...
 BEGIN
     ...
     -- Look for multiple credit columns to be safe
     v_credits_used := GREATEST(
         COALESCE(v_appt.credits_used, 0), 
         COALESCE(v_appt.credit_used, 0),
         0
     );
     
     v_cashback_used := COALESCE(v_appt.cashback_used, 0);

     -- Check for external credit transaction records
     SELECT EXISTS (
         SELECT 1 FROM public.credit_transactions 
         WHERE appointment_id = p_appointment_id 
         AND type IN ('usage', 'appointment_payment', 'used')
     ) INTO v_has_credit_trans;

     -- Calculate Pix amount
     -- If pix_amount is explicitly stored, use it
     IF v_appt.pix_amount IS NOT NULL AND v_appt.pix_amount > 0 THEN
         v_pix_amount := v_appt.pix_amount;
     ELSE
         -- Otherwise calculate it based on total minus what was covered by other means
         v_pix_amount := GREATEST(0, COALESCE(v_appt.total_price, 0) - v_credits_used - v_cashback_used - COALESCE(v_appt.discount_amount, 0));
     END IF;

     -- A PIX payment is considered "paid" if:
     -- 1. Status is paid/confirmed/completed
     -- 2. AND (method contains pix OR there's a remaining amount that must have been paid via PIX to reach 'paid' status)
     ...
     RETURN jsonb_build_object(
         ...
         'has_used_credits', v_credits_used > 0 OR v_has_credit_trans,
         ...
         'requires_financial_decision', v_pix_paid OR v_credits_used > 0 OR v_has_credit_trans OR v_cashback_used > 0,
         'status', v_appt.status,
         'payment_status', v_appt.payment_status,
         'total_price', COALESCE(v_appt.total_price, 0)
     );
 END;
 $function$;
...
BEGIN
    ...
    SET 
        ...
        updated_at = now(),
        refund_status = CASE 
            WHEN NOT v_is_pix_paid THEN 'completed' -- completed because credits were already reverted
    ...
    IF v_is_pix_paid AND p_refund_preference = 'credits' THEN
        ...
        UPDATE public.customers 
        SET credits = COALESCE(credits, 0) + v_pix_amount,
            updated_at = now()
        WHERE id = v_appt.customer_id;
