-- 1. Fix get_or_create_automation function (removed name column)
...
BEGIN
    ...
    IF v_id IS NULL THEN
        -- We ignore p_name because the 'automations' table does not have a 'name' column
        -- (The 'automation_templates' table does have it, which might be the source of confusion)
...
-- 2. Make trigger_appointment_automation robust (preventing appointment blocks)
...
BEGIN
    BEGIN -- Outer block for safety
        ...
        -- We wrap this in another BEGIN/EXCEPTION to be extra safe
        BEGIN
        ...
        LOOP
            IF v_workflow.active THEN
                ...
                EXCEPTION WHEN OTHERS THEN
                    -- Ignore queueing errors to save the appointment
                END;
            END IF;
        END LOOP;
        
    EXCEPTION WHEN OTHERS THEN
        -- Log critical error to console (will appear in Supabase logs) but don't block
